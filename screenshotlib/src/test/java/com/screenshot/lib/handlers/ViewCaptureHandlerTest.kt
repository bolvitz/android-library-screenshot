...
class ViewCaptureHandlerTest {
     ...
     * Simplified for performance - only essential operations
     */
    private fun attachView(view: View, width: Int = 100, height: Int = 100) {
        ...
        // Single measure and layout pass
        rootView.measure(
            View.MeasureSpec.makeMeasureSpec(1000, View.MeasureSpec.EXACTLY),
            View.MeasureSpec.makeMeasureSpec(1000, View.MeasureSpec.EXACTLY)
        )
        ...
        // Force explicit dimensions on the child view if needed
        ...
        // Single looper idle at the end
        shadowOf(Looper.getMainLooper()).idle()
    }

    /**
     * Simplified attachment for tests that don't need full layout
     */
    private fun simpleAttach(view: View) {
        rootView.addView(view)
    }
    ...
    fun `factory returns correct handler for TextView`() {
        val view = TextView(activity)
        // No need to attach for type selection test
        simpleAttach(view)

        // Disable validation since we're only testing handler type selection
    ...
    fun `factory returns correct handler for WebView`() {
        ...
        // WebView in Robolectric is stubborn about dimensions, but we only need the view instance
        simpleAttach(view)
    ...
    fun `getCompatibleHandlers returns all compatible handlers`() {
        val view = ImageView(activity)
        // No need for full attach since we're not validating
        simpleAttach(view)
