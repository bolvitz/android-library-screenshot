...
object ViewCaptureHandlerFactory {
    ...
    // Lazy initialization to avoid creating handlers until needed
    ...
    fun getHandler(view: View, validateVisibility: Boolean = true): ViewCaptureHandler {
        ...
        // Check view type directly for fast common cases before iterating handlers
        return when (view) {
            is ImageView -> ImageViewCaptureHandler()
            is WebView -> WebViewCaptureHandler()
            is VideoView -> VideoViewCaptureHandler()
            is TextureView -> TextureViewCaptureHandler()
            else -> {
                // For less common types (PlayerView, etc.), check handlers
                handlers.firstOrNull { it.canHandle(view) }
