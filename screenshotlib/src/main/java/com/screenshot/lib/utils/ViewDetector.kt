...
object ViewDetector {
    ...
    private fun detectBestView(rootView: View): View {
        // Search within app content view to avoid capturing system UI elements
        // android.R.id.content is the FrameLayout that holds the user's content view
        // Get the first child of android.R.id.content which is the actual app content
        val searchRoot = if (rootView is ViewGroup) {
            val contentFrame = rootView.findViewById<ViewGroup>(android.R.id.content)
            val appContent = if (contentFrame != null && contentFrame.childCount > 0) {
        ...
        // Try to find visible media views within app content only
        findVisiblePlayerView(searchRoot)?.let { return it }
        findVisibleTextureView(searchRoot)?.let { return it }
        findVisibleWebView(searchRoot)?.let { return it }
        findVisibleVideoView(searchRoot)?.let { return it }
        findVisibleImageView(searchRoot)?.let { return it }
